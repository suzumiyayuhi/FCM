    ...
    def plot(result):
        ...
        for i in range(result.__len__()):
            ...
            for j in range(result[i].__len__()):
                plt.plot(result[i][j][2], result[i][j][3], ColorSelect.color_select(i))
                #绘图横纵坐标取自数据的第3和第4维度
        plt.show()
        
    def center_plot(result,x):
        # 画出聚类中心
        plt.rcParams['font.sans-serif']=['SimHei']
        plt.title("第"+str(x)+"次迭代后的聚类中心")
        plt.xlim(0, 10)
        plt.ylim(0, 5)
        plt.xlabel("x")
        plt.ylabel("y")
...
def fuzzy(data, cluster_number, m):
	...
	while (True):
		...
		for j in range(0, cluster_number):
			...
			temC[temNo].append(current_cluster_center)
			temNo=temNo+1
		print("迭代次数:"+str(epoch))
		print("聚类中心:"+str(C))
		PlotUtils.center_plot(temC,epoch)
		# 创建一个距离向量, 用于计算U矩阵。
		distance_matrix =[]
		for i in range(0, len(data)):
			current = []
    ...
    for k in range(0, 3):
        ...
        for i in range(0,3):
            if checker[i]==max(checker):
                ...
                break
        for i in range(0,50):
            if final_location[i + (50*k)][flag] != 1:
                print("分类错误的数据序号："+str(i+(50*k)))
...
#	print_matrix(final_location)
	...
	# 制作画图数据
	result = make_result(final_location,data)